...
zlim([-0.6 1.1]);
print('figures/safety_headon', '-dpng')
figure;
...
zlim([-0.6 1.1]);
print('figures/mission_headon', '-dpng')
figure;
surf(Y,X,rho_colreg');
ylabel('Maneuver Angle [deg]');
xlabel('Obstacle Speed [m/s]');
...
zlim([-1.1 1.1]);
print('figures/colreg_headon', '-dpng')

%% Animate single scenario
[~,i] = min(abs(man_angle-(-20)));
[~,j] = min(abs(speed-15));
[headon overtaking overtaken standon giveway] = colreg_selector( ...
                                                    dataset(i,j).cpa, dataset(i,j).tcpa, ...
                                                    dataset(i,j).eta, dataset(i,j).obs_eta{1});
...
visualizeSim(dataset(i,j), online_safety, online_colreg, online_mission, 10, [0 1000], [-150 150], 'figures/headon_safety_falsification');